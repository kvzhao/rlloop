...
from dqn.environment import GymEnvironment, SimpleGymEnvironment, GlobalIceEnvironment
...
# Environment
flags.DEFINE_string('env_name', 'IceTrainerEnv-v0', 'The name of gym environment to use')
...
def main(_):
  ...
  with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    if config.env_type == 'simple':
      env = GlobalIceEnvironment(config)
