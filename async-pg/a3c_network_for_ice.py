...
class GameACFFNetwork(GameACNetwork):
  def __init__(self,
    ...
    with tf.device(self._device), tf.variable_scope(scope_name) as scope:
      ...
      self.W_fc1, self.b_fc1 = self._fc_variable([256, 256])
      ...
      h_conv2_flat = tf.reshape(h_conv2, [-1, 256])
...
class GameACLSTMNetwork(GameACNetwork):
  def __init__(self,
    ...
    scope_name = "net_" + str(self._thread_index)
    with tf.device(self._device), tf.variable_scope(scope_name) as scope:
      self.W_conv1, self.b_conv1 = self._conv_variable([3, 3, 4, 16])  
      self.W_conv2, self.b_conv2 = self._conv_variable([5, 5, 16, 32]) 
